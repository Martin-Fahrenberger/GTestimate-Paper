...
```{r load_data}
umi_tools_cm <- readRDS('../data/umi_tools_corrected_deep_counts.RDS')
...
write.csv2(umi_tools_dgC_large, file = '../data/cta-seq_ultra_deep.csv')
```
...
#my_cell_ordering <- my_cells
my_cell_ordering <- count_summary_tbl %>% filter(Dataset == 'typical') %>% arrange(`#reads`) %>% pluck('cells')

count_summary_tbl <- count_summary_tbl %>%
  mutate(cells = factor(cells, levels = my_cell_ordering), Dataset = factor(Dataset, levels = c('ultra-deep', 'typical'))) %>%
...
count_plot <- ggplot(count_summary_tbl) +
  geom_line(aes(x = cells, y = `#reads`, linetype = Dataset, group = Dataset, col = Dataset)) +
  geom_point(aes(x = cells, y = `#reads`, col = Dataset), size = .7) +
  ...
  guides(linetype = guide_legend(override.aes = list(color = c('black', 'gray50'))))
...
feature_plot <- ggplot(count_summary_tbl) +
  geom_line(aes(x = cells, y = Feature_count, linetype = Dataset, group = Dataset, col = Dataset)) +
  geom_point(aes(x = cells, y = Feature_count, col = Dataset), size = .7) +
  scale_linetype(breaks = c('typical', 'ultra-deep')) +
  scale_x_discrete(labels = 1:18) +
  scale_color_manual(values = c('gray50', 'black'), guide = 'none') +
  guides(linetype = guide_legend(override.aes = list(color = c('black', 'gray50'))))
...
mm_plot <- ggplot(count_summary_tbl) +
  geom_line(aes(x = cells, y = missing_mass, linetype = Dataset, group = Dataset, col = Dataset)) +
  geom_point(aes(x = cells, y = missing_mass, col = Dataset), size = .7) +
  scale_linetype(breaks = c('typical', 'ultra-deep')) +
  scale_x_discrete(labels = 1:18) +
  scale_color_manual(values = c('gray50', 'black'), guide = 'none') +
  guides(linetype = guide_legend(override.aes = list(color = c('black', 'gray50'))))
...
read_summary_plot <- ggplot(read_summary_tbl) + 
  geom_line(aes(x = cells, y = `raw read count`, linetype = Dataset, group = Dataset, col = Dataset)) +
  geom_point(aes(x = cells, y = `raw read count`, col = Dataset), size = .7) +
  scale_linetype(breaks = c('typical', 'ultra-deep')) +
...
# Relative gene expression estimation error
...
estimation_error_tbl <- estimation_error_tbl %>%
  mutate(cell = factor(cell, levels = my_cell_ordering))  %>%
...
cell_sizes <- read_summary_tbl %>%
  filter(Dataset == 'typical') %>%
  select(cell_barcodes, `raw read count`) %>%
  mutate(cell_barcodes = str_remove(cell_barcodes, pattern = '-1'))
