...
read_count_hist <- read_count_hist +
  ...
  ylab('Cells') +
  xlab('UMIs/cell') +
  ggtitle('UMI count per Cell, pbmc3k') +
  scale_x_continuous(labels=trans_format(trans = 'identity', math_format(phantom()^phantom(8)*.x^phantom(8))), limits = c(0, 7000)) +
...
```{r pbmc_mm}
mm_hist <- readRDS(file = 'downstream_analysis_results/mm_hist.Rds')
mm_hist <- mm_hist +
  theme_classic() +
  theme(axis.title = element_text(size = font_axis),
        axis.text = element_text(size = font_tick)) + 
