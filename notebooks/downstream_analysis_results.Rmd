...
NKG7_expression_tbl <- tibble(genes = rownames(pbmc_seurat_ML), log_expression = GetAssayData(pbmc_seurat_ML, assay = 'RNA', layer = 'data')['NKG7',], method = 'NormalizeData')
NKG7_expression_tbl <- NKG7_expression_tbl %>% rbind(tibble(genes = rownames(pbmc_seurat_GT), log_expression = GetAssayData(pbmc_seurat_GT, assay = 'GTestimate', layer = 'data')['NKG7',], method = 'GTestimate'))
NKG7_expression_tbl <- NKG7_expression_tbl %>% rbind(tibble(genes = rownames(pbmc_seurat_SC), log_expression = GetAssayData(pbmc_seurat_SC, assay = 'SCT', layer = 'data')['NKG7',], method = 'SCTransform'))
```

SCTransform completely changes the scale of the log-expression values and is therefore not comparable:

```{r}

expression_scale_plot <- ggplot(NKG7_expression_tbl %>% filter(log_expression != 0), aes(x = log_expression, fill = method)) + geom_histogram(position = 'dodge')

expression_scale_plot
ggsave(filename = str_glue('{folder}expression_scale_plot.png'), expression_scale_plot, width = 7, height = 3.5)
saveRDS(file = str_glue('{folder}expression_scale_plot.Rds'), expression_scale_plot)
```
